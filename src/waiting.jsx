...
const WaitingLists = () => {
  ...
  const rows = {
    ...
    P: [
      { svc: "Visita endocrinologica", avg: 68, max: 85, target: 120, ok: true },
    ...
    <Section bg="var(--bi-bg-alt)" id="liste-attesa">
      ...
      <div style={{
        ...
        <div>
          {active.map((r, i) => (
            <div key={i} style={{
              ...
              <div style={{
                fontWeight: 600, color: "var(--bi-ink-900)", fontSize: 15,
                display: "flex", alignItems: "center", gap: 10,
              }}>
                <Icon name="stethoscope" size={16} color="var(--bi-ink-500)" />
                {r.svc}
              </div>
              <div style={{ position: "relative", height: 8, background: "var(--bi-ink-100)", borderRadius: 4, overflow: "hidden" }}>
                <div style={{
              ...
              <div style={{ textAlign: "right", fontWeight: 700, color: r.ok ? "var(--bi-success)" : "var(--bi-warm)" }}>
                {r.avg} gg
              </div>
              <div style={{ textAlign: "right", color: "var(--bi-ink-500)", fontSize: 14 }}>
                {r.target} gg
              </div>
            </div>
          ))}
        ...
        <div style={{
          ...
          fontSize: 13, color: "var(--bi-ink-500)",
        }}>
          <div style={{ display: "flex", gap: 20 }}>
            <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ width: 12, height: 3, background: "var(--bi-success)", borderRadius: 2 }} /> Entro soglia
            </span>
            <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ width: 12, height: 3, background: "var(--bi-warm)", borderRadius: 2 }} /> Oltre soglia
            </span>
            <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ width: 2, height: 12, background: "var(--bi-ink-700)" }} /> Soglia regionale
            </span>
          </div>
...
Object.assign(window, { WaitingLists });
