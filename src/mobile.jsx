...
import { Badge, Section, SectionHeading } from './ui.jsx';
import { IOSDevice } from '../frames/ios-frame.jsx';
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
          {[
            {
              icon: 'search',
              title: 'Ricerca persistente',
              desc: 'Barra di ricerca sempre visibile nella bottom nav',
            },
            {
              icon: 'calendar',
              title: 'CUP in un tap',
              desc: "Shortcut al CUP e all'Area Personale nel thumb-zone",
            },
            {
              icon: 'accessibility',
              title: 'Accessibile AA',
              desc: 'Contrasto 7:1, touch target ≥44px, screen reader ready',
            },
          ].map((b, i) => (
          ...
          {[
            {
              icon: 'shield',
              title: 'SPID & CIE',
              desc: 'Login forte integrato nella webapp, no redirect esterni',
            },
            {
              icon: 'download',
              title: 'Installabile (PWA)',
              desc: 'Aggiungi alla home per un accesso offline alle info essenziali',
            },
            {
              icon: 'globe',
              title: 'Multilingua',
              desc: "Italiano, inglese, arabo, ucraino per facilitare l'accesso",
            },
          ].map((b, i) => (
            <div key={i} style={{ display: 'flex', gap: 16 }}>
              <div
...
const MobileContent = () => (
  ...
  >
    ...
    <div
      style={{
        padding: '20px 16px',
        background: 'linear-gradient(160deg, var(--bi-primary-050), var(--bi-surface))',
      }}
    >
      <div
        style={{
      ...
      >
        ...
        <span style={{ fontSize: 13, color: 'var(--bi-ink-500)', flex: 1 }}>
          Cerca un servizio…
        </span>
      </div>
    </div>
    ...
    <div style={{ padding: '0 16px 16px' }}>
      ...
      >
        <div className="placeholder-img" style={{ height: 110, fontSize: 10 }}>
          PLACEHOLDER · foto
        </div>
